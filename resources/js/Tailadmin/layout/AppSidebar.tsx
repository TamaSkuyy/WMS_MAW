...
// Define Menu type matching our DB structure
type Menu = {
  id: number;
  name: string;
  icon?: string;
...
const AppSidebar: React.FC = () => {
  ...
  // Menus from backend
  const dbMenus = useMemo(() => (props.menus as Menu[]) || [], [props.menus]);
  ...
  const renderMenuItems = (items: Menu[], menuType: "main" | "others") => (
    <ul className="flex flex-col gap-4">
      {items.map((nav, index) => (
        <li key={nav.id}>
          {nav.sub_menus && nav.sub_menus.length > 0 ? (
            <button
          ...
          {nav.sub_menus && nav.sub_menus.length > 0 && (isExpanded || isHovered || isMobileOpen) && (
            <div
              ref={(el) => {
  ...
  return (
    <aside
      className={`fixed mt-16 flex flex-col lg:mt-0 top-0 px-5 left-0 bg-white text-[#1A1D23] h-screen transition-all duration-300 ease-in-out z-50 border-r border-[#E9ECEF] shadow-sm
        ${
    ...
    >
      <div
        className={`px-6 py-6 border-b border-[#F1F3F5] mb-2 flex ${
      ...
      <div className="flex flex-col overflow-y-auto duration-300 ease-linear no-scrollbar">
        <nav className="mb-6">
          <div className="flex flex-col gap-4">
            <div>
              <h2
                className={`mb-4 text-[10px] uppercase tracking-[0.15em] text-[#6C757D] font-semibold px-3 flex leading-[20px] ${
                  !isExpanded && !isHovered
                    ? "lg:justify-center"
                    : "justify-start"
                }`}
